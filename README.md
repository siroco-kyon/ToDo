...
- 基準線の保存と比較表示
- 進捗シグナル表示（バーの色で区別。期間のうち今日までに過ぎた日数の割合を「予定の進捗」とし、実際の進捗が予定より何日分遅れているかで判定。「何日分遅れたら遅れにするか」と「土日・祝日を除いて数えるか」は表示設定で変更できる）
  - 順調（青）
  - 遅れ（橙。「約3日遅れ」のように日数で表示）
  - 期限超過（赤。期限から今日までを斜線で表示）
  - 開始前（灰）
- ガントの見やすさ
  - 土日・祝日の列の色分け（祝日名はヘッダーのツールチップ）と、今日の列を囲む赤い枠
  - タスク名・担当・進捗をバーの上に表示。開始日が画面の左外にあっても、見えている範囲の左端に出し続ける
  - マウスを乗せた行を、左の一覧からバーまで横一列でハイライト
  - バーのツールチップに担当・予定との差・最新の進捗ログ、要相談があるタスクにはバーの右に印
  - カテゴリ別／担当者別の並び替え（担当者別はサーバー版）。タスクは表のように続けて並べ、まとまりの境目は色付きの見出しと左端の線、タスクの境目は区切り線で示す。サブタスクは「├ └」で字下げし、背景を少し暗くする
  - 表示単位は日・週・月・年。初期表示と「今日に戻る」では、今日を左から 1/4 の位置に表示
- 今日の計画ビュー
...
- `概要`
  完了率、期限、作業記録、カテゴリ別進捗を1画面で確認します。メンバーごとの活動欄には、期間内の進捗ログを返信まで全文で表示します。
- `報告`
  定例ミーティング用の一覧です。担当者別（またはカテゴリ別）に、タスク・サブタスク・期限・進捗と、選んだ期間の進捗ログ（返信込み）・メモを全文で並べます。最終報告日時から「期間内に報告あり／期間内の報告なし／14日以上報告なし」を色分けし、期間中の進捗の変化（例: 40% → 60%（+20））と期限の延期回数も、タスク・サブタスクごとに表示します（期間中に追加したサブタスクには「期間中に追加」と出ます）。ステータス・開始日・期限・メモの変更、進捗率の変更（バーをマウスで左右にドラッグ）、サブタスクの追加と、完了・期限・進捗の変更、進捗ログの投稿・編集・削除・返信、進捗ログと返信への「いいね」（押した人の表示つき）をこの画面で行えます。定例で話したい進捗ログには「要相談」の印を付けられ、担当者ごとの先頭にまとめて表示されます（相談が済んだら外します）。Markdown コピーと CSV 出力に対応しています。
- `記録`
  作業ログを確認します。
- `詳細`
  タスクの説明、日付、カテゴリ、サブタスク、依存関係、ログを確認・編集します。

補足: ガントを中心にしつつ、用途に応じて概要・カンバン・進捗・報告・計画へ切り替えられます。
