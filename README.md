...
- `報告`
  定例ミーティング用の一覧です。担当者別（またはカテゴリ別）に、タスク・サブタスク・期限・進捗と、選んだ期間の進捗ログ（返信込み）・メモを全文で並べます。最終報告日時から「期間内に報告あり／期間内の報告なし／14日以上報告なし」を色分けします。ステータス・開始日・期限・進捗率・メモの変更、進捗ログの投稿・編集・削除、進捗ログと返信への「いいね」（押した人の表示つき）をこの画面で行えます。Markdown コピーと CSV 出力に対応しています。
- `記録`
  作業ログを確認します。
- `詳細`
