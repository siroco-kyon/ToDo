...
- `報告`
  定例ミーティング用の一覧です。担当者別（またはカテゴリ別）に、タスク・サブタスク・期限・進捗と、選んだ期間の進捗ログ（返信込み）・メモを全文で並べます。最終報告日時から「期間内に報告あり／期間内の報告なし／14日以上報告なし」を色分けし、期間中の進捗の変化（例: 40% → 60%（+20））と期限の延期回数も表示します。ステータス・開始日・期限・メモの変更、進捗率の変更（バーをマウスで左右にドラッグ）、サブタスクの完了・期限・進捗の変更、進捗ログの投稿・編集・削除・返信、進捗ログと返信への「いいね」（押した人の表示つき）をこの画面で行えます。定例で話したい進捗ログには「要相談」の印を付けられ、担当者ごとの先頭にまとめて表示されます（相談が済んだら外します）。Markdown コピーと CSV 出力に対応しています。
- `記録`
  作業ログを確認します。
- `詳細`
